...
# calculate = function(K_ind,K_snp,N_grm,M_grm,fn_y1,fn_y2,dout){
    ...
    dout = '/z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/1.pointest'
    setwd(dout)
...
# for(i in 1:4){
#     for(j in (i+1):5){
#         pipeline_d = c('#!/bin/bash\n',paste0('/s/bin/Rscript /z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/1.pointest.r ',id[i],' ',id[j]))
#         write.table(pipeline_d,paste0(dsh,count,".sh"),row.names=F, col.names=F, quote=F, sep="\n")
#         count = count+1
#     }
# }


# t = paste0(dsh, "condor_submit_settings.sh")
...
# system(paste0("chmod a+x ", dsh, "*.sh"))
# setwd(dsh)
