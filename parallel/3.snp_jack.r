...
# calculate = function(K_ind,K_snp,N_grm,M_grm,fn_y1,fn_y2,dout){
    ...
    #   B_indblock = matrixB(grm_indblock,y_indblock,npara,N-N_indblock,n_cores)
    ...
    dout = '/z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/3.snpjack'
    ...
    #   blocks_snp = blocks_snp_all[k_snp]
      ...
      dosage.f = paste0('/z/Comp/lu_group/Members/jsong/Sibling/coding/dosage/all_ordered/line1_rmed/chr',CHR,'.dosage.gz')
      ...
      command = paste0('/z/Comp/lu_group/Software/plink/plink2_linux_x86_64_20190708/plink2 --fam ',fam.f,' --map ',map.f,' --import-dosage ',dosage.f,' noheader --make-rel square --out ',grmblock.f)
      system(command)
      ## read block GRM 
      ...
      dout = paste0('/z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/3.snpjack/est/',pheno_y1,'_',pheno_y2)
    if(!dir.exists(dout)){
        dir.create(file.path(dout))
    }
    setwd(dout)
    write.table(A_snpblock,paste0('A_',k_snp,'.txt'),quote=F,row.names=F,col.names=F,sep='\t')
    write.table(B_snpblock,paste0('B_',k_snp,'.txt'),quote=F,row.names=F,col.names=F,sep='\t')
    write.table(est_snpblock ,paste0('est_',k_snp,'.txt'),quote=F,row.names=F,col.names=F,sep='\t')
...
for(i in 1:4){
    for(j in (i+1):5){
        ...
        # count = 1
        # for(CHR in 1:22){
        #     files_chr = list.files(path = '/z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/3.snpjack/map',pattern = paste0('chr',CHR,'_block*'))
        #     for(block in 1:length(files_chr)){
        #         pipeline_d = c('#!/bin/bash\n',paste0('/s/bin/Rscript /z/Comp/lu_group/Members/jsong/Sibling/coding/UKB/3.snpjack.r ',id[i],' ',id[j],' ',CHR,' ',block,' ',count))
        #         write.table(pipeline_d,paste0(dsh_ij,count,".sh"),row.names=F, col.names=F, quote=F, sep="\n")
        #         count = count+1
        #     }
        # }
        ...
        # cat("Environment = PYSPARK_SUBMIT_ARGS=\"\"--driver-memory i=18g pyspark-shell\"\"\n", file = t, append = T)
        cat("plusone = $(Process) + 1\n", file = t, append = T)
        cat("NewProcess = $INT(plusone)\n", file = t, append = T)
        cat("executable = $(NewProcess).sh\n", file = t, append = T)
        cat("Request_memory = 200G\n", file = t, append = T)
        cat("Request_cpus = 5\n", file = t, append = T)
